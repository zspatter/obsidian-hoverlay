...
- **Embedded players (auto mode):** media links (YouTube, Vimeo, Spotify, SoundCloud) load the provider's embedded player instead of the full page: lighter, no cookie walls, playlist and timestamp parameters preserved. Toggleable, and any host can be forced back to the full page via per-domain modes.
